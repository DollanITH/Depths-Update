...
public class BlockFlattening implements IFixableData {
	...
	public NBTTagCompound fixTagCompound(final NBTTagCompound compound) {
		...
		try {
			...
			for (int tileEntityIndex = 0; tileEntityIndex < tileEntities.tagCount(); tileEntityIndex++) {
				...
				if (!tileEntityNBT.getKeySet().isEmpty()) {
	...
	static class FlatteningDefinition {
		...
		FlatteningDefinition(final String oldModID, final String oldName, final int oldMetadata, final Block newBlock, final BlockStateGetter blockStateGetter, @Nullable final TileEntityProcessor tileEntityProcessor) {
			this(new ResourceLocation(oldModID, oldName), oldMetadata, newBlock, blockStateGetter, tileEntityProcessor);
		}
	}
